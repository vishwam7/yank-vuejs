...
exports.protect = catchAsync(async(req, res, next) => {
    ...
    if (!token) {
        return next(new AppErr('You are not logged in! Please login to get access.', 401));
    }
    ...
    if (!currentUser) {
        return next(new AppErr('The user belong to this token no longer exists!', 401));
    }

    //4) Check if user change password after the token was issued
    if (currentUser.changedPasswordAfter(decoded.iat)) {
        return next(new AppErr('User recently changed password! Please login again.', 401))
    };
...
exports.restrictTo = (...roles) => {
    return (req, res, next) => {
        ...
        if (!roles.includes(req.user.role)) {
            return next(new AppErr('You are not authorized to perform this kind of Actions!', 403));
        }
...
exports.forgotPassword = catchAsync(async(req, res, next) => {
    ...
    try {
        ...
        await user.save({ validateBeforeSave: false });
        return next(new AppErr('There was an error sending an email. Try again later!', 500));
    }
});
